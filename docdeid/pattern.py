...
class TokenPattern(ABC):
    """
    A pattern that can be applied to a token, and possibly its neighbours, by matching
    the :class:`.Token` text.
    ...
    def match(self, token: Token, metadata: MetaData) -> Optional[tuple[Token, Token]]:
        ...
        :class:`.TokenPattern` should implement the logic of the pattern in this method.
        For example, by checking if the text is lowercase, titlecase, longer than a
        certain number of characters, etc. The :class:`.Token` neighbours may be
        accessible by :meth:`.Token.previous` and :meth:`.Token.next`, if linked by the
        tokenizer.
        ...
        Returns:
            A tuple with the start and end :class:`.Token` if matching, or ``None`` if
